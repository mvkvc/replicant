...
import { calculateWorkerID } from "./crypto";
...
export async function startWorker(
  ...
  console.log("ARG1", modelName, apiKey, salt);
  ...
  try {
    console.log("ARG", apiKey, salt);
    const workerID = calculateWorkerID(apiKey, salt);
    ...
    if (
      ...
      tagsJson.models.find((model: Model) => model.name === modelName) ===
        undefined
    ) {
