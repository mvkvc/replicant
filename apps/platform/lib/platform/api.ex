defmodule Platform.API do
  ...
  def update_rate_limit(email, new_limit) when is_binary(email) do
    case Accounts.get_user_by_email(email) do
      %User{} = changeset -> Repo.update(changeset, rate_limit: new_limit)
      nil -> {:error, :not_found}
    end
  end

  # def update_rate_limit(email, new_limit) when is_binary(email) and is_integer(new_limit) do
  #   case Accounts.get_user_by_email(email) do
  #     %User{} = user ->
  #       user
  #       |> User.changeset(%{rate_limit: new_limit})
  #       |> Repo.update()
